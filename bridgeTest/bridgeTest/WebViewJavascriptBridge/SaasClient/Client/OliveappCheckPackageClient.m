...
const NSString * BASE_URL = @"http://api-lanlingdai.yitutech.com";
const NSString * URL = @"/face/basic/check_image_package";
@interface OliveappCheckPackageClient()
...
- (id) init {
    if (self = [super init]) {
        ...
        _mCheckPackageUrl = [[NSURL alloc] initWithString:fullUrl];
    }
...
- (id) initWithUrl:(NSString *)verifyUrl {
    if (self = [super init]) {
        NSString * fullUrl = [NSString stringWithFormat:@"%@%@",verifyUrl, URL];
        _mCheckPackageUrl = [[NSURL alloc] initWithString:fullUrl];
    }
    _mConnectionTimeoutMilliSec = @(10000);
    _mReadTimeoutLimitMilliSec = @(5000);
...
- (OliveappCheckPackageResult *) verifyByFullEncryptData: (NSData*) imageContent
                                           ...
                                           withThreshold: (OliveappThresholdHelper *) threshold
    ...
    //防屏幕翻拍阈值
    [requestParam setObject:[OliveappThresholdHelper getThreshold:threshold.antiScreenThreshold] forKey:@"query_image_package_check_anti_screen_threshold"];
